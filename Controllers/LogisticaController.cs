...
namespace MinicoreLogisticaAndrade.Controllers
{
    public class LogisticaController : Controller
    {
        ...
        // POST: Logistica/Index
        [HttpPost]
        public IActionResult Index(ReporteLogisticaViewModel viewModel)
        {
            // Validar que FechaFin sea mayor o igual a FechaInicio
            if (viewModel.FechaFin < viewModel.FechaInicio)
            {
                ModelState.AddModelError("FechaFin", "La fecha fin debe ser mayor o igual a la fecha de inicio.");
                return View(viewModel);
            }
            ...
            // incluir navegación a Repartidor y Zona
